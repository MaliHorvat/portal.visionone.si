...
type KerberosEvent = {
  ...
  clipUrl: string;
  client: { id: string; slug: string | null; name: string };
  camera: { id: string; name: string; ip: string; kerberosCameraKey?: string } | null;
...
function statusClass(status: string) {
  ...
  if (status === "offline") return "bg-red-500/15 text-red-300";
  return "bg-slate-500/15 text-slate-300";
}

export function KerberosHubView({ dbConfigured }: { dbConfigured: boolean }) {
  const [edges, setEdges] = useState<KerberosEdge[]>([]);
  const [events, setEvents] = useState<KerberosEvent[]>([]);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [edgeRes, eventRes] = await Promise.all([
        fetch("/api/vms/edges?provider=kerberos", { credentials: "include", cache: "no-store" }),
        fetch("/api/vms/events?provider=kerberos&take=75", { credentials: "include", cache: "no-store" }),
      ]);
      const edgeData = (await edgeRes.json().catch(() => ({}))) as { edges?: KerberosEdge[]; error?: string };
      const eventData = (await eventRes.json().catch(() => ({}))) as { events?: KerberosEvent[]; error?: string };
      if (!edgeRes.ok) throw new Error(edgeData.error ?? "Branje Kerberos agentov ni uspelo.");
      if (!eventRes.ok) throw new Error(eventData.error ?? "Branje Kerberos dogodkov ni uspelo.");
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-[var(--vo-fg)]">Kerberos Hub</h1>
          <p className="mt-1 text-sm text-[var(--vo-muted)]">
            Ločen centralni pogled za Kerberos Agent naprave pri strankah. Ne uporablja plačljivega Kerberos Hub-a.
          </p>
        </div>
        <button
          type="button"
      ...
      <section className="rounded-xl border border-[var(--vo-border)] bg-[var(--vo-surface)] p-5 shadow-[var(--vo-card-shadow)]">
        <h2 className="text-sm font-semibold text-[var(--vo-fg)]">Kerberos agenti</h2>
        {loading ? (
          <p className="mt-3 text-sm text-[var(--vo-muted)]">Nalaganje…</p>
        ) : edges.length === 0 ? (
          <p className="mt-3 text-sm text-[var(--vo-muted)]">Ni povezanih Kerberos agentov.</p>
        ) : (
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {edges.map((edge) => (
              <div key={edge.id} className="rounded-xl border border-[var(--vo-border)] bg-[var(--vo-bg)] p-4">
                ...
                {edge.lastError ? <p className="mt-3 text-xs text-amber-200">{edge.lastError}</p> : null}
              </div>
            ))}
          </div>
        )}
      ...
      <section className="rounded-xl border border-[var(--vo-border)] bg-[var(--vo-surface)] p-5 shadow-[var(--vo-card-shadow)]">
        <h2 className="text-sm font-semibold text-[var(--vo-fg)]">Zadnji Kerberos dogodki</h2>
        {events.length === 0 ? (
              ...
              <tbody>
                {events.map((event) => (
                  <tr key={event.id} className="border-b border-[var(--vo-border)]">
                    ...
                    <td className="py-2 pr-4 text-[var(--vo-fg)]">{event.label || event.eventType}</td>
                    <td className="py-2 pr-4">
                      {event.clipUrl ? (
                        <a className="text-[var(--vo-accent)] hover:underline" href={event.clipUrl} target="_blank">
                          posnetek
                        </a>
                      ) : event.snapshotUrl ? (
