#!/bin/bash
# Ista vsebina kot opt/visionone-agent/install.sh — na boot particiji za firstrun.
set -eu
...
# Agent mora biti že skopiran v /opt/visionone-agent ob firstrun (iz ZIP opt mape)
if [ -f /boot/firmware/opt/visionone-agent/visionone_agent.py ]; then
  cp -r /boot/firmware/opt/visionone-agent/* /opt/visionone-agent/ 2>/dev/null || true
fi
chmod +x /opt/visionone-agent/visionone_agent.py 2>/dev/null || true
