...
import { createFrigateAgentBundleForClient } from "@/lib/rpi-agent-bundle";
...
export async function POST(request: Request, ctx: Ctx) {
  ...
  try {
    ...
    const { zip, meta, filename } = await createFrigateAgentBundleForClient(
      clientId,
      session.username,
      portalBase,
    );
    ...
    console.error(e);
    const msg = e instanceof Error ? e.message : "Napaka pri generiranju Frigate paketa.";
