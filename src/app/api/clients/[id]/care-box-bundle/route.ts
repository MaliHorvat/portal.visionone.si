...
import { createCareBoxBundleForClient } from "@/lib/care-box-bundle";
...
export async function POST(request: Request, ctx: Ctx) {
  ...
  try {
    ...
    const { zip, meta, filename } = await createCareBoxBundleForClient(
      clientId,
      session.username,
      portalBase,
    );
    ...
    console.error(e);
    const msg = e instanceof Error ? e.message : "Napaka pri generiranju Care Box paketa.";
