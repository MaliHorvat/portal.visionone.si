import { buildMojPreventiveItems, type MojPreventiveItem } from "@/lib/client-preventive";
...
export type MojOverview = {
  ...
  preventiveItems: MojPreventiveItem[];
...
export async function getMojOverview(session?: PortalSessionPayload): Promise<MojOverview> {
  ...
  if (!client) {
    return { client: null, upcomingReminders: [], preventiveItems: [], openRequests: [], recentRequests: [] };
  }

  const [reminders, requests] = await Promise.all([
    listRemindersForSession(session, client.id, { clientVisibleOnly: true }),
  ...
  const upcomingReminders = reminders
    ...
    .slice(0, 8);
  const preventiveItems = buildMojPreventiveItems(client, upcomingReminders);
  const recentRequests = [...forClient]
    .sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime())
    .slice(0, 10);

  return { client, upcomingReminders, preventiveItems, openRequests, recentRequests };
}
